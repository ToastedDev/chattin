...
export function Message({ message }: { message: MessageType }): JSX.Element {
  ...
  return (
    <div ref={ref} className="px-3 py-0.5">
      <img
        src={message.author.avatar}
        className="w-4 h-4 rounded-full inline mr-1.5 mb-0.5"
      />
      <p
        className="inline font-bold data-[is-moderator=true]:text-blue-500 data-[is-owner=true]:text-yellow-500"
        data-is-moderator={message.author.badges.moderator}
        data-is-owner={message.author.badges.owner}
      >
        {message.author.name}
        {message.author.badges.moderator && (
          <svg
            className="w-4 h-4 inline fill-current ml-1 mb-0.5"
          ...
          if (urlRegex.test(part)) {
            return (
              <a
                ...
                target="_blank"
                onContextMenu={() => {
                  window.electron.ipcRenderer.send("context-menu:link", part);
                }}
              >
                {part}
              </a>
    ...
    useEffect(() => {
      ...
      async function startChatStream() {
        ...
        if (!videoId && searchParams.channelId) {
          videoId = await window.electron.ipcRenderer.invoke(
            "yt:latest-stream",
      ...
      <>
        {showScrollIndicator && (
          <button
            ...
            onClick={() => {
              chatElemRef.current!.scrollTo({
                top: chatElemRef.current!.scrollHeight,
                behavior: "smooth",
              });
            }}
          >
        ...
        <div
          className="flex-grow overflow-y-auto pb-2"
          ref={chatElemRef}
        >
